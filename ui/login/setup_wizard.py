    ...
    def _wizard_google_link(self):
        ...
        def _on_done(email, refresh_token, error):
            ...
            try:
                ...
                if existing["found"]:
                    _show_dark_warning(self, "\U0001f4c2  Existing Backups Found",
                        f"Found {existing['count']} existing backup(s) in your Google Drive.\n\n"
                        f"Latest: {existing['last_name']} ({existing['last_size']})\n\n"
                        f"You can download from Google Drive > 'Finance Manager Backups' folder.\n"
                        f"Rename to finance.db and place in finance_data/ folder next to the .exe.\n\n"
                        f"Then restart the app to load the restored data.")
            except Exception:
                pass  # Non-critical

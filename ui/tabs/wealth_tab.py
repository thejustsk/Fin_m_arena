...
class _NavWorker(QThread):
    ...
    def __init__(self, url, parent=None):
        super().__init__(parent); self._url = url
    def run(self):
        try:
...
def _clear_layout(layout):
    while layout.count():
        ...
        if w: w.deleteLater()
        child = item.layout()
        if child: _clear_layout(child)

def _metric_card(label, value, color=None):
    color = color or C["text"]
    card = QFrame()
    card.setStyleSheet(f"QFrame{{background:{C['surface']};border:1px solid {C['border2']};border-radius:10px;}}QLabel{{background:transparent;border:none;}}")
    lay = QVBoxLayout(card); lay.setContentsMargins(14,10,14,10); lay.setSpacing(4)
    v = QLabel(value); v.setStyleSheet(f"font-size:18px;font-weight:800;color:{color};")
    l = QLabel(label); l.setStyleSheet(f"font-size:10px;color:{C['text3']};font-weight:600;text-transform:uppercase;letter-spacing:0.5px;")
    lay.addWidget(v); lay.addWidget(l)
    return card

def _badge(text, color):
    lbl = QLabel(text)
    lbl.setStyleSheet(f"color:white;background:{color};border-radius:12px;padding:3px 10px;font-size:11px;font-weight:700;border:none;")
...
def _wealth_card(title, subtitle, amount_text, badge_text, badge_color,
    ...
    card.setStyleSheet(f"QFrame{{background:rgba({r},{g},{b},0.06);border:1.5px solid {badge_color};border-radius:12px;}}QFrame:hover{{background:rgba({r},{g},{b},0.10);border-color:{badge_color};}}QLabel{{background:transparent;border:none;outline:none;}}")
    if on_click: card.setCursor(QCursor(Qt.PointingHandCursor))
    ...
    top.addWidget(t, 1); top.addWidget(_badge(badge_text, badge_color))
    lay.addLayout(top)
    mid = QHBoxLayout()
    s = QLabel(subtitle); s.setStyleSheet(f"font-size:12px;color:{C['text3']};"); s.setWordWrap(True)
    mid.addWidget(s, 1)
    a = QLabel(amount_text); a.setStyleSheet(f"font-size:15px;font-weight:800;color:{C['text']};")
    mid.addWidget(a)
    lay.addLayout(mid)
    if progress_pct is not None:
        ...
        bf = QFrame(); pct = max(0,min(100,int(progress_pct)))
        bf.setStyleSheet(f"background:{badge_color};border-radius:3px;")
        bl.addWidget(bf, pct); bl.addStretch(max(1,100-pct))
        lay.addWidget(bar_bg)
    if extra_line:
        e = QLabel(extra_line); e.setStyleSheet(f"font-size:11px;color:{C['text3']};"); lay.addWidget(e)
    if on_click:
...
def _account_combo(repo):
    cb = QComboBox()
    for a in repo.list_active(): cb.addItem(f"{a['display_name']} ({a['account_type']})", a["account_id"])
    return cb

def _method_combo(repo):
    cb = QComboBox()
    for m in repo.list_methods(): cb.addItem(m["display_name"], m["method_id"])
    return cb
...
def _person_combo(list_fn, id_keys):
    ...
    for p in list_fn():
        pid = None
        for k in id_keys:
            if p.get(k): pid = p[k]; break
        if pid is not None: cb.addItem(p["name"], pid)
    return cb

def _category_id(db, preferred_names, fallback=None):
    for name in preferred_names:
        r = db.execute("SELECT category_id FROM categories WHERE LOWER(display_name)=LOWER(?) AND is_active=1", (name,)).fetchone()
        if r: return r["category_id"]
...
def _log_ledger_txn(tx_repo, db, *, account_id, pay_method, tx_type, amount,
    ...
    try:
        return tx_repo.create(tx_date=TODAY(), account_id=account_id, pay_method=pay_method,
        ...
        print(f"[WARN] Ledger txn failed: {e}"); return None

def _build_subnav(container_layout, labels):
    nav = QHBoxLayout(); nav.setSpacing(8)
    btns = [QPushButton(l) for l in labels]
    for b in btns: b.setMinimumHeight(32); b.setCursor(QCursor(Qt.PointingHandCursor)); nav.addWidget(b)
    nav.addStretch(); container_layout.addLayout(nav)
    stack = QStackedWidget(); container_layout.addWidget(stack)
    def goto(i): _switch_tabs(btns, i); stack.setCurrentIndex(i)
    for i, b in enumerate(btns): b.clicked.connect(lambda _, i=i: goto(i))
    _switch_tabs(btns, 0)
    return stack, btns

def _build_history_shell(container_lay, sort_options):
    stats_row = QHBoxLayout(); container_lay.addLayout(stats_row)
    top = QHBoxLayout()
    ...
    top.addWidget(sort_cb); top.addStretch(); container_lay.addLayout(top)
    table = QTableWidget(); table.setEditTriggers(QAbstractItemView.NoEditTriggers)
    table.verticalHeader().setVisible(False); container_lay.addWidget(table, 1)
    return stats_row, sort_cb, table

def _fill_stats_row(row_layout, cards):
    for c in cards: row_layout.addWidget(c)
...
class NavFetchDialog(QDialog):
    def __init__(self, initial_query="", parent=None):
        ...
        self.result_nav = None; self.result_name = None; self._matches = []
        ...
        row.addWidget(self.query_box, 1); row.addWidget(search_btn); lay.addLayout(row)
        ...
        info.setStyleSheet(f"color:{C['text3']};font-size:11px;"); lay.addWidget(info)
        btn_row = QHBoxLayout()
        cancel = QPushButton("Cancel"); cancel.clicked.connect(self.reject)
        btn_row.addStretch(); btn_row.addWidget(cancel); lay.addLayout(btn_row)
        if initial_query: self._search()
    ...
    def _on_search_result(self, data):
        self.results.clear()
        if not data: self.results.addItem("No matches found."); return
        matches = data[:30] if isinstance(data, list) else []
        for m in matches: self.results.addItem(f"{m.get('schemeName','?')}  [{m.get('schemeCode','?')}]")
        self._matches = matches

    def _pick(self, item):
        idx = self.results.row(item)
        if not self._matches or idx >= len(self._matches): return
        m = self._matches[idx]
        ...
        self._nav_worker.error.connect(lambda e: QMessageBox.warning(self, "Fetch Failed", f"Couldn't fetch NAV ({e})."))
    ...
    def _on_nav_result(self, data, m):
        ...
        if nav is None: QMessageBox.warning(self, "No Data", "No NAV data available."); return
        self.result_nav = nav; self.result_name = m.get("schemeName"); self.accept()


class _FunctionPage(QWidget):
    ICON = "\U0001f4b0"; TITLE = "Function"
    def __init__(self, repos, services, parent=None):
        super().__init__(parent)
        self.repos = repos; self.services = services; self.db = repos["accounts"].db
        self._build_skeleton()
    def _build_skeleton(self):
        lay = QVBoxLayout(self); lay.setContentsMargins(0,8,0,0); lay.setSpacing(12)
        hdr = QLabel(f"{self.ICON}  {self.TITLE}")
        hdr.setStyleSheet(f"font-size:16px;font-weight:800;color:{C['text']};"); lay.addWidget(hdr)
        nav = QHBoxLayout(); nav.setSpacing(8)
        self.btn_entry = QPushButton("\uff0b Entry")
        self.btn_list = QPushButton("\U0001f4cb List")
        self.btn_history = QPushButton("\U0001f550 History")
        self._sub_btns = [self.btn_entry, self.btn_list, self.btn_history]
        for b in self._sub_btns: b.setMinimumHeight(32); b.setCursor(QCursor(Qt.PointingHandCursor)); nav.addWidget(b)
        nav.addStretch(); lay.addLayout(nav)
        self.sub_stack = QStackedWidget(); lay.addWidget(self.sub_stack, 1)
        ...
        _switch_tabs(self._sub_btns, 1); self.sub_stack.setCurrentIndex(1)
    def _goto(self, idx):
        _switch_tabs(self._sub_btns, idx); self.sub_stack.setCurrentIndex(idx)
        if idx == 0: self._refresh_entry_dropdowns()
        elif idx == 1: self.load_list()
        elif idx == 2: self.load_history()
    def refresh(self):
    ...
    def _scroll_area(self):
        ...
        scroll.setWidget(inner); return scroll, v
...
class LoansGivePage(_FunctionPage):
    ICON = "\U0001f91d"; TITLE = "Loans I Give"
    def _build_entry(self):
        ...
        f1.addRow("Name *", self.lg_borrower_name); f1.addRow("", add_b)
        ...
        give_btn = QPushButton("\U0001f91d  Give Loan"); give_btn.setObjectName("primary")
        ...
        rep_btn = QPushButton("\U0001f4b0  Log Repayment"); rep_btn.setObjectName("primary")
    ...
    def _refresh_entry_dropdowns(self):
        ...
        for b in self.repos["loans"].list_borrowers(): self.lg_loan_borrower.addItem(b["name"], b["borrower_id"])
        self.lg_rep_loan.clear()
        for l in self.repos["loans"].list_loans():
            if l["status"] != "CLOSED": self.lg_rep_loan.addItem(f"{l['borrower_name']} — {fmt_money(l['loan_amount'])} ({l['status']})", l["loan_id"])
        self._update_lg_pending_label()
    def _update_lg_pending_label(self):
        lid = self.lg_rep_loan.currentData()
        if not lid: self.lg_rep_pending_lbl.setText(""); return
    ...
    def _add_borrower(self):
        ...
        if not name: QMessageBox.warning(self, "Missing Name", "Please enter the borrower's name."); return
        self.repos["loans"].create_borrower(name); self.lg_borrower_name.clear()
        self._refresh_entry_dropdowns()
        QMessageBox.information(self, "Added", f"'{name}' added to your borrower directory.")
    def _give_loan(self):
        bid = self.lg_loan_borrower.currentData(); amount = self.lg_loan_amount.value()
        if not bid or amount <= 0: QMessageBox.warning(self, "Missing Info", "Select a borrower and enter an amount."); return
        account_id = self.lg_loan_account.currentData(); method = self.lg_loan_method.currentData()
        borrower_name = self.lg_loan_borrower.currentText()
        txn_id = _log_ledger_txn(self.repos["transactions"], self.db, account_id=account_id, pay_method=method,
        ...
        self.repos["loans"].create_loan(borrower_id=bid, loan_amount=amount, payment_method=method,
    ...
    def _log_repayment(self):
        lid = self.lg_rep_loan.currentData(); amount = self.lg_rep_amount.value()
        if not lid or amount <= 0: QMessageBox.warning(self, "Missing Info", "Select a loan and enter an amount."); return
        loan = self.repos["loans"].get_loan(lid)
        account_id = self.lg_rep_account.currentData(); method = self.lg_rep_method.currentData()
        txn_id = _log_ledger_txn(self.repos["transactions"], self.db, account_id=account_id, pay_method=method,
        ...
        self.repos["loans"].add_repayment(loan_id=lid, amount_paid=amount,
    ...
    def _build_list(self):
        page = QWidget(); lay = QVBoxLayout(page)
        scroll, self.lg_list_lay = self._scroll_area(); lay.addWidget(scroll); return page
    def load_list(self):
        self.repos["loans"].sync_overdue(); _clear_layout(self.lg_list_lay)
        ...
        for l in loans:
            ...
            card = _wealth_card(title=l["borrower_name"],
                ...
                progress_pct=pct, extra_line=f"Loan: {fmt_money(l['loan_amount'])} {MDOT} Repaid: {fmt_money(paid)}",
    ...
    def _build_history(self):
        page = QWidget(); lay = QVBoxLayout(page)
        self.lg_stats_row, self.lg_sort_cb, self.lg_hist_table = _build_history_shell(lay, ["Status", "Loan ID", "Person"])
        self.lg_sort_cb.currentIndexChanged.connect(self.load_history)
        self.lg_hist_table.setColumnCount(6)
        self.lg_hist_table.setHorizontalHeaderLabels(["Borrower", "Loan Amount", "Repaid", "Pending", "Status", "Due Date"])
        self.lg_hist_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)
        return page
    def load_history(self):
        self.repos["loans"].sync_overdue(); _clear_layout(self.lg_stats_row)
        loans = self.repos["loans"].list_loans()
        total_pending = sum(max(l["loan_amount"] - self.repos["loans"].total_repaid(l["loan_id"]), 0) for l in loans if l["status"] != "CLOSED")
        ...
        if sort_mode == "Status": loans.sort(key=lambda l: rank.get(l["status"], 9))
        elif sort_mode == "Loan ID": loans.sort(key=lambda l: l["loan_id"])
        else: loans.sort(key=lambda l: l["borrower_name"])
...
class LoansTakePage(_FunctionPage):
    ICON = "\U0001f3db\ufe0f"; TITLE = "Loans I Take"
    def _build_entry(self):
        page = QWidget(); lay = QVBoxLayout(page)
        ...
        f1.addRow("Name *", self.lt_lender_name); f1.addRow("", add_l)
        ...
        take_btn = QPushButton("\U0001f3db\ufe0f  Take Loan"); take_btn.setObjectName("primary")
        take_btn.clicked.connect(self._take_loan)
        f2.addRow("", take_btn)
        self.lt_entry_stack.addWidget(p2); self._update_emi_preview()
        p3 = QWidget(); f3 = QFormLayout(p3)
        ...
        pay_btn = QPushButton("\U0001f4b8  Log EMI Payment"); pay_btn.setObjectName("primary")
    ...
    def _update_emi_preview(self):
        ...
        else: self.lt_emi_preview.setText("Estimated EMI: —")
    def _refresh_entry_dropdowns(self):
        self.lt_loan_lender.clear()
        for l in self.repos["borrowed"].list_lenders(): self.lt_loan_lender.addItem(l["name"], l["lender_id"])
        self.lt_rep_loan.clear()
        for l in self.repos["borrowed"].list_loans():
            if l["status"] != "CLOSED": self.lt_rep_loan.addItem(f"{l['lender_name']} — {fmt_money(l['principal_amount'])} ({l['status']})", l["loan_id"])
    ...
    def _prefill_emi_amount(self):
        ...
        if loan and loan.get("emi_amount"): self.lt_rep_amount.setValue(loan["emi_amount"])
    def _add_lender(self):
        name = self.lt_lender_name.text().strip()
        if not name: QMessageBox.warning(self, "Missing Name", "Please enter the lender's name."); return
        self.repos["borrowed"].create_lender(name); self.lt_lender_name.clear()
        self._refresh_entry_dropdowns()
        QMessageBox.information(self, "Added", f"'{name}' added to your lender directory.")
    def _take_loan(self):
        lid = self.lt_loan_lender.currentData(); principal = self.lt_loan_principal.value()
        months = self.lt_loan_months.value(); rate = self.lt_loan_rate.value()
        if not lid or principal <= 0: QMessageBox.warning(self, "Missing Info", "Select a lender and enter the principal."); return
        emi = LoanService.emi(principal, rate, months)
        start = self.lt_loan_start.date().toPyDate(); due = _add_months(start, months)
        account_id = self.lt_loan_account.currentData(); method = self.lt_loan_method.currentData()
        ...
        self.repos["borrowed"].create_loan(lender_id=lid, principal_amount=principal, interest_rate=rate, emi_amount=emi,
        ...
        self._refresh_entry_dropdowns(); self.load_list(); self.load_history()
        QMessageBox.information(self, "Loan Recorded", f"\u20b9{principal:,.2f} loan from {lender_name}. EMI \u2248 {fmt_money(emi)}/mo.")
    def _log_emi_payment(self):
        lid = self.lt_rep_loan.currentData(); amount = self.lt_rep_amount.value()
        if not lid or amount <= 0: QMessageBox.warning(self, "Missing Info", "Select a loan and enter an amount."); return
        loan = self.repos["borrowed"].get_loan(lid)
        account_id = self.lt_rep_account.currentData(); method = self.lt_rep_method.currentData()
        txn_id = _log_ledger_txn(self.repos["transactions"], self.db, account_id=account_id, pay_method=method,
            ...
            category_names=("Finance", "Other"))
        self.repos["borrowed"].add_repayment(loan_id=lid, amount_paid=amount,
    ...
    def _build_list(self):
        page = QWidget(); lay = QVBoxLayout(page)
        scroll, self.lt_list_lay = self._scroll_area(); lay.addWidget(scroll); return page
    def load_list(self):
        self.repos["borrowed"].sync_overdue(); _clear_layout(self.lt_list_lay)
        ...
        for l in loans:
            ...
            subtitle = f"Rate {l['interest_rate']}% {MDOT} EMI {fmt_money(l['emi_amount'])} {MDOT} Due {l['due_date'] or EM_DASH}"
            card = _wealth_card(title=l["lender_name"], subtitle=subtitle,
    ...
    def _mark_closed(self, loan_id):
        if _confirm(self, "Mark Closed", "Mark this loan as fully closed?"):
            ...
            self.load_list(); self.load_history(); return True
        return False
    def _build_history(self):
        page = QWidget(); lay = QVBoxLayout(page)
        self.lt_stats_row, self.lt_sort_cb, self.lt_hist_table = _build_history_shell(lay, ["Status", "Loan ID", "Person"])
        self.lt_sort_cb.currentIndexChanged.connect(self.load_history)
        self.lt_hist_table.setColumnCount(6)
        self.lt_hist_table.setHorizontalHeaderLabels(["Lender", "Principal", "EMI", "Repaid", "Status", "Due Date"])
        self.lt_hist_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)
        return page
    def load_history(self):
        self.repos["borrowed"].sync_overdue(); _clear_layout(self.lt_stats_row)
        loans = self.repos["borrowed"].list_loans()
        active = [l for l in loans if l["status"] != "CLOSED"]
        total_pending = sum(max(l["principal_amount"] - self.repos["borrowed"].total_repaid(l["loan_id"]), 0) for l in active)
        ...
        else: loans.sort(key=lambda l: l["lender_name"])
...
class LoanDetailDialog(QDialog):
    def __init__(self, role, title, principal, status, start_date, due_date, description,
        ...
        hdr.addWidget(_badge(status, color)); lay.addLayout(hdr)
        info_bits = [f"Principal: {fmt_money(principal)}", f"Start: {start_date}", f"Due: {due_date or EM_DASH}"]
        if rate is not None: info_bits.insert(1, f"Rate: {rate}%")
        if emi is not None: info_bits.insert(2, f"EMI: {fmt_money(emi)}")
        info = QLabel(f" {MDOT} ".join(info_bits))
        info.setStyleSheet(f"color:{C['text3']};font-size:12px;"); info.setWordWrap(True); lay.addWidget(info)
        if description:
        ...
        summary = QLabel(f"Total Repaid: {fmt_money(total_paid)}  {MDOT}  Remaining: {fmt_money(max(principal - total_paid, 0))}")
        summary.setStyleSheet(f"color:{C['text']};font-weight:700;font-size:12px;padding-top:6px;"); lay.addWidget(summary)
        if amortization:
            tabs = QTabWidget()
        ...
        btn_row.addWidget(ok); lay.addLayout(btn_row)
    def _repayments_table(self, repayments, amount_key, date_key):
        table = QTableWidget(); table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        table.verticalHeader().setVisible(False); table.setColumnCount(3)
    ...
    def _amortization_table(self, schedule):
        table = QTableWidget(); table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        table.verticalHeader().setVisible(False); table.setColumnCount(5)
...
class FDGivePage(_FunctionPage):
    ICON = "\U0001f3e6"; TITLE = "FD I Deposit"
    def _build_entry(self):
        ...
        for w in (self.fd_principal, self.fd_rate): w.valueChanged.connect(self._update_maturity_preview)
        ...
        create_btn = QPushButton("\U0001f3e6  Create Fixed Deposit"); create_btn.setObjectName("primary")
        create_btn.clicked.connect(self._create_fd); f.addRow("", create_btn)
    ...
    def _update_maturity_preview(self):
        ...
        else: self.fd_maturity_preview.setText("Estimated Maturity Amount: —")
    def _refresh_entry_dropdowns(self): pass
    def _create_fd(self):
        p = self.fd_principal.value()
        if p <= 0: QMessageBox.warning(self, "Missing Info", "Please enter the deposit principal."); return
        if self.fd_maturity.date() <= self.fd_start.date(): QMessageBox.warning(self, "Invalid Dates", "Maturity date must be after start date."); return
        account_id = self.fd_account.currentData(); account_name = self.fd_account.currentText()
        default_method = self.repos["lookups"].list_methods()
        method_id = default_method[0]["method_id"] if default_method else None
        txn_id = _log_ledger_txn(self.repos["transactions"], self.db, account_id=account_id, pay_method=method_id,
            tx_type="DEBIT", amount=p, person_org=None,
            description=f"FD deposit at {account_name}", category_names=("Investment", "Finance"))
        self.repos["fd"].create(bank_account_id=account_id, principal_amount=p, interest_rate=self.fd_rate.value(),
        ...
        self.fd_principal.setValue(0); self.load_list(); self.load_history()
        QMessageBox.information(self, "FD Created", "Fixed deposit recorded successfully.")
    def _build_list(self):
        page = QWidget(); lay = QVBoxLayout(page)
        scroll, self.fd_list_lay = self._scroll_area(); lay.addWidget(scroll); return page
    def load_list(self):
        self.repos["fd"].sync_matured(); _clear_layout(self.fd_list_lay)
    ...
    def _mark_matured(self, fd_id):
        if _confirm(self, "Mark Matured", "Mark this FD as matured?"):
            self.repos["fd"].update_status(fd_id, "MATURED"); self.load_list(); self.load_history(); return True
        return False
    def _mark_withdrawn(self, fd_id, account_id, method_id):
        ...
        self.repos["fd"].update_status(fd_id, "WITHDRAWN"); self.load_list(); self.load_history(); return True
    def _build_history(self):
        page = QWidget(); lay = QVBoxLayout(page)
        self.fd_stats_row, self.fd_sort_cb, self.fd_hist_table = _build_history_shell(lay, ["Status", "FD ID", "Maturity Date"])
        self.fd_sort_cb.currentIndexChanged.connect(self.load_history)
        self.fd_hist_table.setColumnCount(6)
        self.fd_hist_table.setHorizontalHeaderLabels(["Account", "Principal", "Rate", "Maturity Amount", "Status", "Maturity Date"])
        self.fd_hist_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)
        return page
    def load_history(self):
        self.repos["fd"].sync_matured(); _clear_layout(self.fd_stats_row)
        fds = self.repos["fd"].list_all()
        ...
        if sort_mode == "Status": fds.sort(key=lambda f: rank.get(f["status"], 9))
        elif sort_mode == "FD ID": fds.sort(key=lambda f: f["fd_id"])
        else: fds.sort(key=lambda f: f["maturity_date"])
...
class FDDetailDialog(QDialog):
    def __init__(self, fd, on_mark_matured, on_mark_withdrawn, accounts_repo, lookups_repo, parent=None):
        ...
        self.accounts_repo = accounts_repo; self.lookups_repo = lookups_repo
        lay = QVBoxLayout(self)
        hdr = QHBoxLayout()
        ...
        pb.setFormat(f"{pct:.0f}% of tenure elapsed"); lay.addWidget(pb)
        info = QLabel(f"Principal: {fmt_money(fd['principal_amount'])}  {MDOT}  Rate: {fd['interest_rate']}%  {MDOT}  "
                       f"Maturity Amount: {fmt_money(fd['maturity_amount'] or 0)}\n"
                       f"Start: {fd['start_date']}  {MDOT}  Maturity: {fd['maturity_date']}")
        info.setStyleSheet(f"color:{C['text2']};font-size:12px;"); info.setWordWrap(True); lay.addWidget(info)
        btn_row = QHBoxLayout(); btn_row.addStretch()
        if fd["status"] == "ACTIVE" and pct >= 100:
            m_btn = QPushButton("\u2705 Mark Matured"); m_btn.setObjectName("primary")
            m_btn.clicked.connect(self._do_mark_matured); btn_row.addWidget(m_btn)
        if fd["status"] in ("ACTIVE", "MATURED"):
            w_btn = QPushButton("\U0001f4b5 Mark Withdrawn (credit maturity amount)")
            w_btn.clicked.connect(self._do_mark_withdrawn); btn_row.addWidget(w_btn)
        ok = QPushButton("Close"); ok.clicked.connect(self.accept)
        btn_row.addWidget(ok); lay.addLayout(btn_row)
    def _do_mark_matured(self):
        if self._on_matured(): self.accept()
    def _do_mark_withdrawn(self):
        ...
        if idx >= 0: acc_cb.setCurrentIndex(idx)
        method_cb = _method_combo(self.lookups_repo)
        f.addRow("Credit Into *", acc_cb); f.addRow("Method *", method_cb)
        row = QHBoxLayout()
        ...
        if dlg.exec_() == QDialog.Accepted:
            if self._on_withdrawn(acc_cb.currentData(), method_cb.currentData()): self.accept()


class FDOthersPage(_FunctionPage):
    ICON = "\U0001f9fe"; TITLE = "FD Others Deposit"
    def _build_entry(self):
        page = QWidget(); lay = QVBoxLayout(page)
        ...
        f1.addRow("Name *", self.fo_depositor_name); f1.addRow("", add_d)
        ...
        self.fo_dep_interest_free.setChecked(True); self.fo_dep_interest_free.setObjectName("pill")
        ...
        take_btn = QPushButton("\U0001f9fe  Record Deposit"); take_btn.setObjectName("primary")
        take_btn.clicked.connect(self._create_deposit); f2.addRow("", take_btn)
        ...
        rep_btn = QPushButton("\U0001f4b8  Log Repayment"); rep_btn.setObjectName("primary")
        rep_btn.clicked.connect(self._log_repayment); f3.addRow("", rep_btn)
    ...
    def _toggle_interest_free(self, checked):
        ...
        if checked: self.fo_dep_rate.setValue(0)
    def _refresh_entry_dropdowns(self):
        self.fo_dep_depositor.clear()
        for d in self.repos["deposits"].list_depositors(): self.fo_dep_depositor.addItem(d["name"], d["depositor_id"])
        self.fo_rep_deposit.clear()
        for d in self.repos["deposits"].list_deposits():
            if d["status"] != "CLOSED": self.fo_rep_deposit.addItem(f"{d['depositor_name']} — {fmt_money(d['principal_amount'])} ({d['status']})", d["deposit_id"])
        self._update_fo_pending_label()
    def _update_fo_pending_label(self):
        did = self.fo_rep_deposit.currentData()
        if not did: self.fo_rep_pending_lbl.setText(""); return
    ...
    def _add_depositor(self):
        ...
        if not name: QMessageBox.warning(self, "Missing Name", "Please enter the depositor's name."); return
        self.repos["deposits"].create_depositor(name); self.fo_depositor_name.clear()
        self._refresh_entry_dropdowns()
        QMessageBox.information(self, "Added", f"'{name}' added to your depositor directory.")
    def _create_deposit(self):
        did = self.fo_dep_depositor.currentData(); amount = self.fo_dep_amount.value()
        if not did or amount <= 0: QMessageBox.warning(self, "Missing Info", "Select a depositor and enter an amount."); return
        account_id = self.fo_dep_account.currentData(); method = self.fo_dep_method.currentData()
        ...
        self.repos["deposits"].create_deposit(depositor_id=did, principal_amount=amount, interest_rate=rate,
    ...
    def _log_repayment(self):
        did = self.fo_rep_deposit.currentData(); amount = self.fo_rep_amount.value()
        if not did or amount <= 0: QMessageBox.warning(self, "Missing Info", "Select a deposit and enter an amount."); return
        dep = self.repos["deposits"].get_deposit(did)
        account_id = self.fo_rep_account.currentData(); method = self.fo_rep_method.currentData()
        txn_id = _log_ledger_txn(self.repos["transactions"], self.db, account_id=account_id, pay_method=method,
            ...
            category_names=("Finance", "Other"))
        self.repos["deposits"].add_repayment(deposit_id=did, amount_paid=amount,
    ...
    def _build_list(self):
        page = QWidget(); lay = QVBoxLayout(page)
        scroll, self.fo_list_lay = self._scroll_area(); lay.addWidget(scroll); return page
    def load_list(self):
        _clear_layout(self.fo_list_lay)
        ...
        for d in deps:
            ...
            card = _wealth_card(title=d["depositor_name"],
                subtitle=f"Deposited {d['deposit_date']} {MDOT} Expected return {d['expected_return_date'] or EM_DASH}",
    ...
    def _build_history(self):
        page = QWidget(); lay = QVBoxLayout(page)
        self.fo_stats_row, self.fo_sort_cb, self.fo_hist_table = _build_history_shell(lay, ["Status", "Deposit ID", "Person"])
        self.fo_sort_cb.currentIndexChanged.connect(self.load_history)
        self.fo_hist_table.setColumnCount(6)
        self.fo_hist_table.setHorizontalHeaderLabels(["Depositor", "Principal", "Returned", "Pending", "Rate", "Status"])
        self.fo_hist_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)
        return page
    def load_history(self):
        _clear_layout(self.fo_stats_row)
        deps = self.repos["deposits"].list_deposits()
        active = [d for d in deps if d["status"] != "CLOSED"]
        total_pending = sum(max(d["principal_amount"] - self.repos["deposits"].total_repaid(d["deposit_id"]), 0) for d in active)
        ...
        if sort_mode == "Status": deps.sort(key=lambda d: 0 if d["status"] != "CLOSED" else 1)
        elif sort_mode == "Deposit ID": deps.sort(key=lambda d: d["deposit_id"])
        else: deps.sort(key=lambda d: d["depositor_name"])
...
class MFPage(_FunctionPage):
    ICON = "\U0001f4c8"; TITLE = "Mutual Funds"
    def __init__(self, repos, services, parent=None):
        self._nav_cache = {}
    ...
    def _build_entry(self):
        ...
        self.mf_scheme_type = QComboBox(); self.mf_scheme_type.addItems(["Equity", "Debt", "Hybrid", "Index", "ELSS", "Liquid", "Other"])
        ...
        f1.addRow("AMC *", self.mf_amc); f1.addRow("Scheme Name *", self.mf_scheme_name)
        f1.addRow("Type", self.mf_scheme_type); f1.addRow("Folio Number", self.mf_folio)
        f1.addRow("", add_s)
        ...
        fetch_btn = QPushButton("\U0001f50e Fetch NAV"); fetch_btn.clicked.connect(self._fetch_nav_for_buy)
        ...
        for w in (self.mf_buy_amount, self.mf_buy_nav): w.valueChanged.connect(self._update_units_preview)
        self.mf_buy_account = _account_combo(self.repos["accounts"])
        self.mf_buy_method = _method_combo(self.repos["lookups"])
        f2.addRow("Scheme *", self.mf_buy_scheme); f2.addRow("Type", self.mf_buy_type)
        f2.addRow("Date", self.mf_buy_date); f2.addRow("Amount *", self.mf_buy_amount)
        f2.addRow("NAV *", nav_row); f2.addRow("", self.mf_buy_units_preview)
        f2.addRow("Pay From *", self.mf_buy_account); f2.addRow("Method *", self.mf_buy_method)
        buy_btn = QPushButton("\U0001f4c8  Log Purchase"); buy_btn.setObjectName("primary")
        buy_btn.clicked.connect(self._log_purchase); f2.addRow("", buy_btn)
        ...
        sell_fetch_btn = QPushButton("\U0001f50e Fetch NAV"); sell_fetch_btn.clicked.connect(self._fetch_nav_for_sell)
        ...
        for w in (self.mf_sell_units, self.mf_sell_nav): w.valueChanged.connect(self._update_redemption_preview)
        self.mf_sell_account = _account_combo(self.repos["accounts"])
        self.mf_sell_method = _method_combo(self.repos["lookups"])
        f3.addRow("Scheme *", self.mf_sell_scheme); f3.addRow("", self.mf_holdings_lbl)
        f3.addRow("Date", self.mf_sell_date); f3.addRow("Units to Redeem *", self.mf_sell_units)
        f3.addRow("NAV *", sell_nav_row); f3.addRow("", self.mf_sell_amount_preview)
        f3.addRow("Credit Into *", self.mf_sell_account); f3.addRow("Method *", self.mf_sell_method)
        sell_btn = QPushButton("\U0001f4b5  Log Redemption"); sell_btn.setObjectName("primary")
        sell_btn.clicked.connect(self._log_redemption); f3.addRow("", sell_btn)
    ...
    def _update_holdings_label(self):
        ...
        if not sid: self.mf_holdings_lbl.setText(""); return
    ...
    def _fetch_nav_for_buy(self):
        ...
        if dlg.exec_() == QDialog.Accepted and dlg.result_nav: self.mf_buy_nav.setValue(dlg.result_nav)
    def _fetch_nav_for_sell(self):
        dlg = NavFetchDialog(initial_query=self.mf_sell_scheme.currentText(), parent=self)
        if dlg.exec_() == QDialog.Accepted and dlg.result_nav: self.mf_sell_nav.setValue(dlg.result_nav)
    def _refresh_entry_dropdowns(self):
    ...
    def _add_scheme(self):
        ...
        if not amc or not name: QMessageBox.warning(self, "Missing Info", "Enter both AMC and scheme name."); return
        self.repos["mf"].create_scheme(amc_name=amc, scheme_name=name, scheme_type=self.mf_scheme_type.currentText(),
    ...
    def _log_purchase(self):
        ...
        if not sid or amount <= 0 or nav <= 0: QMessageBox.warning(self, "Missing Info", "Select a scheme and enter amount and NAV."); return
        units = MFService.calculate_units(amount, nav)
        account_id = self.mf_buy_account.currentData(); method = self.mf_buy_method.currentData()
        scheme_label = self.mf_buy_scheme.currentText()
        txn_id = _log_ledger_txn(self.repos["transactions"], self.db, account_id=account_id, pay_method=method,
            tx_type="DEBIT", amount=amount, person_org=None,
            description=f"MF {self.mf_buy_type.currentText().title()} — {scheme_label}", category_names=("Investment", "Finance"))
        self.repos["mf"].add_txn(scheme_id=sid, txn_type=self.mf_buy_type.currentText(),
            txn_date=self.mf_buy_date.date().toString("yyyy-MM-dd"),
            amount=amount, nav=nav, units=units, linked_txn_id=txn_id)
        self._nav_cache[sid] = nav; self.mf_buy_amount.setValue(0)
        self._refresh_entry_dropdowns(); self.load_list(); self.load_history()
        QMessageBox.information(self, "Purchase Logged", f"{units:,.4f} units purchased.")
    def _log_redemption(self):
        sid = self.mf_sell_scheme.currentData()
        units = self.mf_sell_units.value(); nav = self.mf_sell_nav.value()
        if not sid or units <= 0 or nav <= 0: QMessageBox.warning(self, "Missing Info", "Select a scheme and enter units and NAV."); return
        held = self.repos["mf"].holdings(sid)["units"]
        if units > held + 1e-6: QMessageBox.warning(self, "Not Enough Units", f"You only hold {held:,.4f} units."); return
        amount = round(units * nav, 2)
        account_id = self.mf_sell_account.currentData(); method = self.mf_sell_method.currentData()
        scheme_label = self.mf_sell_scheme.currentText()
        txn_id = _log_ledger_txn(self.repos["transactions"], self.db, account_id=account_id, pay_method=method,
            tx_type="CREDIT", amount=amount, person_org=None,
            description=f"MF Redemption — {scheme_label}", category_names=("Investment", "Finance"))
        self.repos["mf"].add_txn(scheme_id=sid, txn_type="REDEMPTION",
            txn_date=self.mf_sell_date.date().toString("yyyy-MM-dd"),
            amount=amount, nav=nav, units=units, linked_txn_id=txn_id)
        self._nav_cache[sid] = nav; self.mf_sell_units.setValue(0)
        self._refresh_entry_dropdowns(); self.load_list(); self.load_history()
    ...
    def _last_nav(self, scheme_id):
        if scheme_id in self._nav_cache: return self._nav_cache[scheme_id]
        txns = self.repos["mf"].list_txns(scheme_id)
        return txns[-1]["nav"] if txns else 0
    def _build_list(self):
        page = QWidget(); lay = QVBoxLayout(page)
        self.mf_portfolio_row = QHBoxLayout(); lay.addLayout(self.mf_portfolio_row)
        scroll, self.mf_list_lay = self._scroll_area(); lay.addWidget(scroll); return page
    def load_list(self):
        _clear_layout(self.mf_list_lay); _clear_layout(self.mf_portfolio_row)
        ...
        for s in schemes:
            ...
            total_invested += net_invested; total_current += current_value
        ...
        _fill_stats_row(self.mf_portfolio_row, [
            ...
            _metric_card("Overall Return", f"{overall_return:+.2f}%", C["green"] if overall_return >= 0 else C["red"])])
        if not schemes:
        ...
        for s in schemes:
            ...
            card = _wealth_card(title=f"{s['amc_name']} — {s['scheme_name']}",
                subtitle=f"{s['scheme_type'] or ''} {MDOT} {h['units']:,.4f} units {MDOT} NAV {nav:,.4f}",
    ...
    def _build_history(self):
        page = QWidget(); lay = QVBoxLayout(page)
        self.mf_stats_row, self.mf_sort_cb, self.mf_hist_table = _build_history_shell(lay, ["Date", "Scheme", "Type"])
        self.mf_sort_cb.currentIndexChanged.connect(self.load_history)
        self.mf_hist_table.setColumnCount(6)
        self.mf_hist_table.setHorizontalHeaderLabels(["Date", "Scheme", "Type", "Amount", "NAV", "Units"])
        self.mf_hist_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        return page
    def load_history(self):
        _clear_layout(self.mf_stats_row)
        ...
        if sort_mode == "Date": all_txns.sort(key=lambda t: t["txn_date"], reverse=True)
        elif sort_mode == "Scheme": all_txns.sort(key=lambda t: t["scheme_label"])
        else: all_txns.sort(key=lambda t: t["txn_type"])
...
class MFDetailDialog(QDialog):
    def __init__(self, scheme, txns, holdings, nav, parent=None):
        ...
        t.setStyleSheet(f"font-size:16px;font-weight:800;color:{C['text']};")
        t.setWordWrap(True); hdr.addWidget(t, 1)
        hdr.addWidget(_badge(f"{ret:+.2f}%", color)); lay.addLayout(hdr)
        info = QLabel(f"Folio: {scheme.get('folio_number') or EM_DASH}  {MDOT}  Type: {scheme.get('scheme_type') or EM_DASH}\n"
                       f"Units Held: {holdings['units']:,.4f}  {MDOT}  Latest NAV: {nav:,.4f}\n"
                       f"Invested: {fmt_money(net_invested)}  {MDOT}  Current Value: {fmt_money(current_value)}")
        info.setStyleSheet(f"color:{C['text2']};font-size:12px;"); lay.addWidget(info)
        table = QTableWidget(); table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        table.verticalHeader().setVisible(False); table.setColumnCount(5)
...
class WealthTab(QWidget):
    ...
    def _build(self):
        ...
        outer.setContentsMargins(32, 24, 32, 24); outer.setSpacing(14)
        heading = QLabel("\U0001f4c8  Wealth")
        heading.setStyleSheet(f"font-size:22px;font-weight:800;color:{C['text']};")
        outer.addWidget(heading)
        group_row = QHBoxLayout(); group_row.setSpacing(8)
        self.btn_people = QPushButton("\U0001f465 People")
        self.btn_deposits_grp = QPushButton("\U0001f3e6 Deposits")
        self.btn_invest_grp = QPushButton("\U0001f4c8 Investments")
        ...
        group_row.addStretch(); outer.addLayout(group_row)
        self.group_stack = QStackedWidget(); outer.addWidget(self.group_stack, 1)
        ...
        self.group_stack.addWidget(self._wrap_group([
            ("\U0001f91d Loans I Give", self.loans_give_page),
            ("\U0001f3db\ufe0f Loans I Take", self.loans_take_page)]))
        self.group_stack.addWidget(self._wrap_group([
            ("\U0001f3e6 FD I Deposit", self.fd_give_page),
            ("\U0001f9fe FD Others Deposit", self.fd_others_page)]))
        self.group_stack.addWidget(self._wrap_group([
            ("\U0001f4c8 Mutual Funds", self.mf_page)]))
        ...
        _switch_tabs(self._group_btns, 0); self.group_stack.setCurrentIndex(0)
    ...
    def _wrap_group(self, function_pages):
        ...
        for b in btns: b.setMinimumHeight(32); b.setCursor(QCursor(Qt.PointingHandCursor)); fn_row.addWidget(b)
        fn_row.addStretch(); lay.addLayout(fn_row)
        stack = QStackedWidget()
        for _, page in function_pages: stack.addWidget(page)
    ...
    def _goto_group(self, i):
        _switch_tabs(self._group_btns, i); self.group_stack.setCurrentIndex(i)

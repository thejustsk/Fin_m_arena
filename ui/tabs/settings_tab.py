    ...
    def _guide_functions(self):
        ...
        sections = [
            ("\U0001f3f7\ufe0f  Core Concepts", "#0EA5E9", [
                ...
                ("Need vs Want", "A behavioural classification for regular personal debit expenses: Need (unavoidable), Want (discretionary), or Not Set (untagged). Income, transfers, wealth movements and full Split ledger payments are Not Applicable. For Split expenses, only your allocated share enters Need/Want analytics using the category and choice recorded in Split."),
            ...
            ("\U0001f3e0  Home", "#4F46E5", [
                ...
                ("Charts", "4 Chart.js charts: Spending by Category (doughnut), Spending Trend (line), Need vs Want (stacked bar), Income vs Expense by Account (horizontal bar)."),
                ("Quick Access", "Tiles that jump straight to the main tabs. Top Spends shows the largest debit spends for the selected period; reminders rotate in a fixed area above Savings Rate."),
            ]),
            ("\U0001f4ca  Budgets", "#D97706", [
                ("Monthly, Yearly & Special", "Create recurring Monthly or Yearly budgets, a selected-period override, or a one-time Special date-range budget. A selected-period budget overrides the matching recurring target without changing future periods."),
                ("Scopes", "Budget Category, Money Purpose, Need Spending, Want Spending, Total Personal Spending, or a Transaction Group. Personal consumption budgets exclude income, transfers and wealth movements; Split budgets count only your allocated share."),
                ("Review & Actions", "Select the month or year to review history. Budget cards show spent, limit, remaining or over-budget value, and can expand to related transactions. Use Edit, Deactivate, Reactivate or Delete. Warnings show current Monthly, Yearly and current-month Special alerts."),
            ]),
            ...
            ("\U0001f4b0  Balances", "#10B981", [
                ("Net Worth", "Hero card (indigo gradient, matching the Wealth dashboard) showing total net worth with breakdown by account type: Current, Credit Card, Wallet, Cash. Credit cards show negative (money owed)."),
            ...
            ("\U0001f91d  Split Expenses", "#7C3AED", [
                ("Groups & Contacts", "Create groups and pick members from your contact directory, or add new contacts inline. You always appear as a member, shown as 'Your Name (You)'."),
                ("Status Card", "Indigo header card showing Owed to you, You owe, and Settled / Unsettled group counts across every group."),
                ("Overview", "3 KPI boxes (Total Expenses, Pending, Settled), a Balance Matrix per member, minimal-transfer Settlement Suggestions, and the combined Transactions list. Click any transaction to edit it."),
                ("Record Expense", "Pick who paid, the amount and date, then split Equally, by Percentage, or with Custom amounts. Creates a linked ledger transaction."),
                ("Record Settlement", "Log a payment between two members. Auto-fills from the settlement suggestions. Also creates a linked ledger transaction."),
                ("Print", "The Print button next to Record Settlement exports the whole Overview for the selected group as a secured PDF — the 3 KPI boxes, Balance Matrix, Settlement Suggestions and every transaction. Requires password/TOTP verification."),
            ]),
            ("\U0001f4b3  Credit Cards", "#EF4444", [
                ...
                ("Reminders", "Right panel: overdue alerts, statement approaching, high-value transactions, card expiry warnings. Up to 15 reminders sorted by urgency. A condensed version appears in the sidebar."),
            ]),
            ...
            ("\u2699\ufe0f  Settings", "#6B7280", [
                ...
                ("Data Management", "Local backup with retention and storage info, plus Google Drive backup (Manual / On App Close / Daily / Weekly with its own retention). Export, Import, Take Down and Cloud Sync are still Coming Soon."),
            ]),
        ]
    ...
    def _guide_scheme(self):
        ...
        sections = [
            ("\U0001f4be  Data Storage", "#4F46E5", [
                ...
                ("Need/Want Storage", "Stored as: 0 = Not Set, 1 = Need, 2 = Want, 3 = Not Applicable. Not Applicable is used for income, transfers and wealth ledger movements. Personal Split shares are derived separately, so the full amount paid for a group is never treated as personal spending."),
            ...
            ("\U0001f510  Security", "#EF4444", [
                ("Password", "Bcrypt-hashed master password. Required for login. Change password from Settings > Security."),
                ("2FA / TOTP", "Optional Time-based One-Time Password. QR code setup with an authenticator app (Google Authenticator, Authy, etc.). When enabled, the 6-digit code replaces the password for login and for verified edits."),
                ("Google OAuth", "Optional alternative login. Links a Google account via browser OAuth flow. Verifies with Google each time (no stored password). Email and refresh token stored locally."),
                ("Tab Security", "Optional per-tab password protection. Each toggle requires password/TOTP verification to change, and the protected tab then prompts on every visit."),
                ("Verified Actions", "Wealth edits, audit edits, changing your display name, and printing a Split group all require password/TOTP confirmation through the same dialog. Cancelling aborts the action."),
            ]),
            ("\U0001f517  Wealth & Split Linking", "#059669", [
                ("Auto-Linking", "Giving a loan, creating an FD, buying MF units, recording a split expense or logging a settlement all automatically create a matching transaction in the ledger. The linked_txn_id column connects the two records."),
            ...
            ("\U0001f4b3  Card Billing (FIFO)", "#D97706", [
                ("Statement Cycles", "Computed from statement_date (e.g. '6th' means cycles run from the 7th of one month to the 6th of the next). Cycles are built going backwards from today."),
                ("FIFO Allocation", "Payments (CREDIT transactions) are allocated to the oldest cycle with a remaining balance first — First In, First Out. Each cycle tracks debits, paid and remaining."),
                ("Amount Due", "Sum of remaining from ALL previous cycles, excluding the current one because its bill hasn't generated yet."),
                ("Due Dates", "Default: cycle end date + grace_days. Editable per cycle in the card details view. Changes are saved to the card_cycles table."),
                ("Reminders", "Statement approaching (5 days before, if balance > 0), overdue (past due date with remaining), high-value transactions (configurable threshold) and card expiry warnings. Shown in the Cards tab and, condensed, in the sidebar."),
            ]),
            ("\U0001f4ca  Charts & Visualization", "#8B5CF6", [
                ("Chart.js", "All charts use Chart.js 4.4.0 loaded via CDN and rendered in a QWebEngineView. HTML is built with data injected as JSON, written to a temp file, then loaded locally. Charts need an internet connection to draw."),
                ...
                ("Monthly Charts", "The same 4 charts for the selected month. A forced resize runs when switching to the Visualization tab (QWebEngineView layout fix)."),
                ("Audit Insights", "Same chart template. Auto-aggregates by month when the date range exceeds 90 days, otherwise by day."),
            ]),
            ("\U0001f4e6  Backup", "#F59E0B", [
                ("Automatic (Local)", "A local backup is written when the app closes. Stored in finance_data/backups/ with a timestamped filename. Retention keeps the most recent 14 (oldest auto-deleted)."),
                ("Manual (Local)", "Settings > Data Management > Backup Now creates an immediate timestamped copy of finance.db."),
                ("Google Drive", "Optional cloud backup. Choose Manual Only, On App Close, Daily (first launch) or Weekly, with its own retention count. Requires linking your Google account."),
                ("Storage", "Database and backup sizes are shown in Settings > Data Management. Backup files are ordinary SQLite databases and open in any SQLite viewer."),
                ("Restore", "Replace finance_data/finance.db with a backup file and restart the app. Note: if finance.db-wal and finance.db-shm files are present, remove them too so the restored file is read cleanly."),
            ]),
        ]
    ...
    def _guide_ui(self):
        ...
        sections = [
            ("\U0001f3e0  Sidebar", "#4F46E5", [
                ("Navigation", "Left sidebar, collapsed to icons by default. Click the header to expand to full labels. Groups: Home, then DAILY (Transactions, Database, Balances, Split), CARDS (Credit Cards, Debit Cards), REVIEW & PLANNING (Audit, Budgets, Wealth), TOOLS (Notes, Settings, Gmail)."),
                ("Need/Want Counter", "Shows the count of untagged transactions (neednwant is NULL). Clicking scrolls to the first untagged transaction in Transaction Entry."),
                ("Payment Dues", "When expanded, credit-card dues appear under PAYMENT DUES as compact two-row cards: card name on top (truncated with a tooltip for the full name), cycle and amount below, a coloured left edge for urgency and a red '!' badge when overdue. The list scrolls vertically and never widens the sidebar. When collapsed, dues shrink to coloured dots."),
                ("Highlight", "The active tab is highlighted with an indigo background, driven by sidebar.highlight(key) from the main window on every tab switch."),
            ]),
            ("\U0001f3a8  Theme", "#8B5CF6", [
                ("Colors", "Light theme with an indigo accent (#4F46E5). Semantic colors: green (income/success), red (expense/danger), amber (warning), text3 (secondary text). All defined in theme.py as the C dict."),
            ...
            ("\U0001f4b3  Carousel Pattern", "#EF4444", [
                ...
                ("Interaction", "Drag to scroll, scroll wheel, Left/Right arrows, Space to flip the nearest card. Click a card to flip it. Click the 'VIEW CARD DETAILS' stripe to open details."),
                ("Timer Management", "showEvent starts the timer, hideEvent stops it. Prevents dual-timer lag when switching between the CC and DC tabs."),
                ("Independence", "CC and DC carousels are completely independent. Zero imports between cards_tab.py and debit_cards_tab.py. Each has its own CardItem, CarouselView and PreviewWidget classes."),
            ]),
            ("\U0001f4f1  Smart Scroll", "#10B981", [
                ("Lazy Loading", "Transaction lists load in batches (configurable page size, default 150). More load when you scroll within 400px of the bottom. Configurable in Settings > Preferences."),
                ("Wealth Cards", "Cards are pre-built in memory then rendered in batches (default 150 per batch), with the scroll handler triggering the next batch."),
                ("Wealth Alerts", "The dashboard's Alerts & Upcoming panel scrolls independently beneath the fixed Net Position bar and KPI grid. It renders at most 150 cards per refresh and shows a '+ N more' note beyond that, so a very large backlog can never freeze the UI."),
                ("DC Transactions", "Smart initial load: 1 month first, expanding if fewer than 4 transactions (up to 6 months), then 3 months per batch. setUpdatesEnabled(False) during widget addition."),
                ("Notes", "Lazy scroll with a configurable page size (default 50) and scroll trigger (default 200px)."),
                ("Audit", "Lazy scroll with a configurable page size. Disconnects the scroll signal once everything is loaded."),
            ]),
            ("\u2328\ufe0f  Keyboard Shortcuts", "#D97706", [
                ("General", "Ctrl+K: open Command Palette for fuzzy tab navigation and actions. Ctrl+Shift+L: toggle Light/Dark theme. Tab: move between fields. Enter: submit form / activate button. Escape: close dialog / cancel."),
                ("Carousel", "Left/Right arrows: scroll carousel. Space: flip nearest card. Scroll wheel: browse cards."),
                ("Transaction Entry", "Enter on the DEBIT button toggles DEBIT/CREDIT. Enter on the Need/Want buttons activates that option. Enter on the Add button submits. Enter is blocked during the transfer animation."),
                ("SearchableCombo", "Type to filter the dropdown. Up/Down arrows navigate filtered results. Enter selects the highlighted item. Escape closes the dropdown."),
                ("Notes", "Tag input: type then Enter to add a tag. Up/Down arrows navigate suggestions. Enter selects a suggestion. Escape hides suggestions."),
            ]),
            ("\U0001f5a8\ufe0f  Printing & PDFs", "#0EA5E9", [
                ("Where", "Monthly statements and filtered results (Database), individual wealth records and pending lists (Wealth), notes (Notes), and a full group overview (Split)."),
                ("Layout", "Indigo header band with the document title, a status badge, an info grid, KPI boxes, then colour-coded sections that mirror the on-screen cards."),
                ("Security", "Every PDF carries a Doc ID, a content hash, a diagonal watermark, a confidential footer and a final verification page with a QR code."),
                ("Requirements", "PDF export needs the reportlab package (and qrcode for the QR panel). If reportlab is missing the app tells you instead of failing silently."),
            ]),
